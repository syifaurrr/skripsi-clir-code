...
def preprocess_fathul_muin(text):
    ...
    # 3. Menghilangkan simbol khusus
    text = text.replace('~~', ' ')
    text = text.replace('#', '')
    text = text.replace('*', '')
    
    # 4. Menghilangkan deretan 0 berulang (000000...)
    text = re.sub(r'0{2,}', '', text)
    
    # 5. Menghilangkan identifier halaman (PageV01P147, ms002, dsb)
    text = re.sub(r'PageV\d+P\d+', '', text)
    text = re.sub(r'ms\d+', '', text)
    
    # 6. Bersihkan spasi & baris kosong berlebih
    text = re.sub(r' +', ' ', text)
    text = re.sub(r'\n +', '\n', text)
    text = re.sub(r'\n{3,}', '\n\n', text)
...
output_file = os.path.join(script_dir, 'fathul_muin2.csv')
